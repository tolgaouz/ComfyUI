...
def mock_expandvars_appdata():
    mock = Mock()
    mock.side_effect = lambda path: path.replace(
...
def test_override_models_dir(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print("test_models_dir", test_models_dir)

        config_data = {
            "comfyui": {
                "is_default": True,
                "models_path": test_models_dir,
                "vae": "vae",
            }
        }
        ...
        try:
            ...
            # Check if models_dir was overridden
            assert folder_paths.models_dir == test_models_dir
            print(folder_paths.folder_names_and_paths["vae"][0][0])
            assert folder_paths.folder_names_and_paths["vae"][0][0] == os.path.join(
                test_models_dir, "vae"
            )
